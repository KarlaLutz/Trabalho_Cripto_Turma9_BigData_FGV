...
Original file is located at
    https://colab.research.google.com/drive/1PDhnAgwarLoo0Vm4VDW9dwHmfzmkbgtR
"""
...
def feature_eng(df):
    ...
    # Um contador de minutos desde o início da série
    df['time']=(df['datetime'].astype(np.int64)/6e10).astype(int)
    cols2drop = set(['symbol', 'datetime', 'close_time', 'open', 'high', 'low', 'forward_average','hora_cheia','open_time']).intersection(set(df.columns))
    df = df.drop(columns=cols2drop)
    df = df.dropna()
    df['time'] = df['time']-26038829
    df = sm.add_constant(df)
